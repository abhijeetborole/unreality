...
//	public Playable		sequenceRef;
	...
	class RigidData {
		...
		public RigidData(MeshRenderer mr) {
			...
			props.SetMatrix(SID_PREV_MVP, Matrix4x4.identity);
	...
	void OnEnable() {
		...
		m_velocityCamera.allowHDR = false;
	...
	void OnPreCull() {
		...
		for(int i = 0, n = ms_dynamicRigidData.Count; i < n; ++i) {
			...
			rd.props.SetMatrix(SID_PREV_MVP, prevMVP);
			rd.props.SetFloat("u_VelocityFramerateFraction", frameRateFraction);
