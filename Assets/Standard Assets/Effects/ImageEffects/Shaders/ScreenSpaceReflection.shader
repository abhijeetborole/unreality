...
Shader "Hidden/ScreenSpaceReflection" 
{
	...
	float applyVignette(float2 tsP, float vignetteSize, float vignetteStrength) {
		float vignette = pow(16.0 * tsP.x * tsP.y * (1.0 - tsP.x) * (1.0 - tsP.y), 3.0 * vignetteSize);
	...
	float4 fragRaytrace(v2f i, int stepRate)
	{
        ...
        float2 invSize = float2(1.0 / _ScreenSize.x, 1.0 / _ScreenSize.y);
        float4 H0 = mul(_ProjectToPixelMatrix, float4(csPosition + (csN) * rayBump, 1.0));
        float k0 = 1.0 / H0.w;
	...
	float4 fragGBlur(v2f i) : SV_Target
	{
        ...
        if (weightSum > 0.01) {
		    ...
		    return float4(resultSum.rgb * invWeightSum, confidence);
        } else {
            return float4((unweightedResultSum / (2 * radius + 1)).rgb, 0.0);
        }
	}
	...
	float4 getEdgeValue(float2 tsP, int mip) {
		float4 coord = float4(tsP + float2(1.0/(2* _ReflectionBufferSize * exp2(-mip))),0,0);
		if (mip == 0) {
	...
	float4 bilateralUpsampleReflection(float2 tsP, int mip) {
		...
		float2 smallTexSize = _ReflectionBufferSize * exp2(-mip);
		...
		float2 p3 = (smallPixelPosi+1) * smallTexSizeInv;
        ...
        float4 temp1 = tex2Dlod(_NormalAndRoughnessTexture, hiP0);
        float4 temp2 = tex2Dlod(_NormalAndRoughnessTexture, hiP0);
        float4 temp3 = tex2Dlod(_NormalAndRoughnessTexture, hiP0);
	...
	{
		float2 tsP = i.uv2.xy;
		float2 lastTexSize = _ReflectionBufferSize * exp2(-_LastMip);
